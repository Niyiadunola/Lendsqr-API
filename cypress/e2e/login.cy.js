describe('Login API Tests', () => {
  ...
  it('Valid Login', () => {
    cy.request('POST', `${baseUrl}/pecunia/api/v2/auth/admin/login`, {
      email: "adunola.adeniyi56@gmail.com", // Use a valid email for login
  ...
  it('Invalid Login - Missing Email', () => {
    cy.request({
      method: 'POST',
      url: `${baseUrl}/pecunia/api/v2/auth/admin/login`,
      body: {
        email: "",  // Missing email
  ...
  it('Invalid Login - Invalid Email', () => {
    cy.request({
      ...
      expect(response.status).to.eq(400);  // Expecting a Bad Request
      expect(response.body).to.have.property('message', 'Invalid Emaill Address');
    });
  });
  ...
  it('Invalid Login - Empty Credentials Email and Password', () => {
    cy.request({
      method: 'POST',
      url: `${baseUrl}/pecunia/api/v2/auth/admin/login`,
      body: {
      ...
      expect(response.status).to.eq(400);  // Expecting Bad Request
      expect(response.body).to.have.property('message', 'email is not allowed to be empty');
    });
  });
